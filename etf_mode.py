...
ETF_MAP = {
    "SPY": "SPY", "VOO": "VOO", "IVV": "IVV", "QQQ": "QQQ",
    "VTI": "VTI", "SCHD": "SCHD", "VXUS": "VXUS", "VEA": "VEA",
    "VWO": "VWO", "IWM": "IWM", "DIA": "DIA", "RSP": "RSP",
    "BND": "BND", "AGG": "AGG", "TLT": "TLT", "IEF": "IEF",
    "LQD": "LQD", "HYG": "HYG", "GLD": "GLD", "SLV": "SLV",
    "VNQ": "VNQ", "XLK": "XLK", "XLF": "XLF", "XLE": "XLE",
    "XLV": "XLV", "XLY": "XLY", "XLP": "XLP", "XLI": "XLI",
    "XLC": "XLC", "XLU": "XLU", "XLB": "XLB", "ARKK": "ARKK",
    "JEPI": "JEPI", "JEPQ": "JEPQ", "VT": "VT", "ACWI": "ACWI",
}
...
def force_snapshot(user_id, value_ghs, mode="etf"):
    try:
...
def safe_price(symbol, price):
    if "etf_price_memory" not in st.session_state:
        st.session_state.etf_price_memory = {}

    memory = st.session_state.etf_price_memory
...
def get_last_good_value():
    return st.session_state.get("etf_last_good_value", None)


def set_last_good_value(value):
    st.session_state.etf_last_good_value = value
...
def load_etf_holdings(user_id):
    holdings = {k: 0.0 for k in ETF_MAP}
...
def save_etf_holdings(user_id, holdings):
    rows = [
        {"user_id": user_id, "symbol": k, "quantity": float(v)}
        for k, v in holdings.items()
    ]

    db().table("etf_holdings").upsert(
        rows,
...
def etf_app():
    ...
    currency_index = load_currency_index(user_id, "etf")
    selected_currency = CURRENCY_OPTIONS[currency_index]

    rate = load_setting(user_id, "etf_rate", 14.5)
    invested = load_setting(user_id, "etf_investment", 0.0)
    cash = load_setting(user_id, "etf_cash", 0.0)

    holdings = load_etf_holdings(user_id)
    ...
    if st.sidebar.button("💾 Save Settings"):
        save_setting(user_id, "etf_currency_index", selected_index)
        save_setting(user_id, "etf_rate", rate)
        save_setting(user_id, "etf_investment", invested)
    ...
    with st.expander("⚙️ Manage ETF Holdings", expanded=False):
        st.caption("Enter ETF units held. The dashboard will show your top 10 ETF holdings by value.")
        ...
        if st.button("💾 Save ETF Holdings"):
            save_etf_holdings(user_id, holdings)
            save_setting(user_id, "etf_cash", cash)
            st.success("ETF holdings saved")

    try:
        prices = stock_live_prices(list(ETF_MAP.keys())) or {}
    except Exception:
        prices = {}
    ...
    if failed_assets:
        st.warning(
            "Some ETF prices could not be refreshed live: "
            + ", ".join(failed_assets)
    ...
    st.subheader("🏆 Top 10 ETF Holdings")

    if top_df.empty:
        st.info("No ETF holdings entered yet.")
    else:
        st.dataframe(top_df, use_container_width=True)

    with st.expander("📂 View All ETF Assets"):
        st.dataframe(
    ...
    if total_value > 0 and not data_degraded:
        autosave_portfolio_value(user_id, total_value, "etf")

    if st.button("Save Snapshot"):
        if total_value > 0 and not data_degraded:
            force_snapshot(user_id, total_value, "etf")
